...
(function () {
  ...
  function MakeMultiFilter(originalArray) {
    ...
    function applyFilters() {
      var result = originalArray.slice();
      for (var i = 0; i < filterFunctions.length; i++) {
        result = filterArray(result, filterFunctions[i]);
      }
      if (callbackFunction) {
        callbackFunction.call(originalArray, result);
      }
      return result;
    }

    function createFilter(filterFunc, callback) {
      filterFunctions.push(filterFunc);
  ...
  if (typeof filterFunc !== 'function') {
    console.error('MakeMultiFilter does not return a function', typeof filterFunc);
    p1Message = 'FAILURE';
  } else {
    ...
    result = filterFunc(function (item) {
      ...
      if (!arraysAreTheSame([1, 2, 3], this)) {
        console.error('filter function callback does not pass the original array as this', this);
        p1Message = 'FAILURE';
      }
    });
